...
The second image shows an image taken from each of the unique traffic sign labels. 
<P>
...
I altered the classic LeNet5 by adding dropout layers and an extra convolution/Relu layer. A dropout of 0.5 was needed after every hidden fully connected layer to reduce overtrainning.This was evident by the rise of the validation accuracy and the very high training accuracy.  The extra convolution layer increased the final test accuracy of the DNN by about 2% by increasing the featuremap depth significantly.
...
The final model accuracy of 96.6% as fairly close to the validation set accuracy of 98.9% which shows that the model is working well, but there is still a tiny bit of overtrainning left. The fact that the training loss is gradual and does not come back up shows that the learnign rate is correct. Overally this shows that the model is working well for the traffic sign classifier problem.
...
Layer one is the output of the first convolution layer nad shows the effect of the weights and biases for each of the 32 filters. There are several 45 degree (9, 16, 20, and 23), horizontal (8, 17, and 17), and vertical (2, 7, 24, and 26) filters. There are also features that look a lot like a threshold was aplied. See 3 and 23. There are also some filters which seem to be composits of the above filters.

<p align="center">
<img src="./vis_layer_1.png" alt="Layer 1" style="width: 800px;"/>
</p>

Layer two has more depth but less width and height.  This layer is harder to understand what is going with these filter.

<p align="center">
<img src="./vis_layer_2.png" alt="Layer 2" style="width: 800px;"/>
</p>

Layer three has very little spacial information but a lot more filters to generalize the classification problem.
